...
AND CUST_GENDER = 'F' 
AND CUST_MARITAL_STATUS IN ('married', 'Married') 
...
SELECT DISTINCT c.CUST_ID, c.CUST_FIRST_NAME, c.CUST_LAST_NAME,  c.CUST_YEAR_OF_BIRTH, p.PROD_LIST_PRICE, p.PROD_NAME
FROM CUSTOMERS c
JOIN SALES s ON c.CUST_ID = s.CUST_ID
JOIN PRODUCTS p ON s.PROD_ID = p.PROD_ID
WHERE (p.PROD_SUBCATEGORY = 'Sweaters - Men' OR p.PROD_SUBCATEGORY = 'Sweaters - Women')
...
AND p.PROD_LIST_PRICE = (SELECT MIN(p2.PROD_LIST_PRICE) 
	...
	WHERE (p2.PROD_SUBCATEGORY = 'Sweaters - Men' OR p2.PROD_SUBCATEGORY = 'Sweaters - Women'))
ORDER BY c.CUST_ID;
...
SELECT c.CUST_ID, c.CUST_FIRST_NAME, c.CUST_LAST_NAME, c.CUST_MARITAL_STATUS, c.CUST_INCOME_LEVEL, c.COUNTRY
FROM CUSTOMERS c
...
AND EXISTS (SELECT 1 
    ...
    AND c2.COUNTRY IN ('United States of America', 'Germany')
...
SELECT SUBSTR(CUST_EMAIL, INSTR(CUST_EMAIL, '@')) AS EMAIL_DOMEN, COUNT(*) AS EMAIL_COUNT
FROM CUSTOMERS c
...
FROM (SELECT s.TIME_ID, p.PROD_CATEGORY, SUM(s.QUANTITY_SOLD) AS DAY_SUM
    FROM SALES s
    JOIN PRODUCTS p ON s.PROD_ID = p.PROD_ID
...
CREATE TABLE SALES_BIGSURMOON_RYZHKOV_EDUARD AS SELECT s.*
FROM SALES s
JOIN TIMES t ON s.TIME_ID = t.TIME_ID
WHERE (t.CALENDAR_YEAR, t.CALENDAR_MONTH_NUMBER) = (
    SELECT t.CALENDAR_YEAR, t.CALENDAR_MONTH_NUMBER
    FROM SALES s2
    JOIN TIMES t ON s2.TIME_ID = t.TIME_ID
...
--SELECT * FROM SALES_BIGSURMOON_RYZHKOV_EDUARD ORDER BY AMOUNT_SOLD DESC;
...
SELECT PROD_ID, CUST_ID, TO_CHAR(TRUNC(TIME_ID) 
	+ (FLOOR(DBMS_RANDOM.VALUE(0, 24)) / 24)
	+ (FLOOR(DBMS_RANDOM.VALUE(0, 60)) / 1440)
	+ (FLOOR(DBMS_RANDOM.VALUE(0, 60)) / 86400),
	'DD.MM.YYYY HH24:MI:SS') AS TIME_ID, CHANNEL_ID, PROMO_ID, QUANTITY_SOLD, AMOUNT_SOLD
